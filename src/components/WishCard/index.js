...
class WishCard extends React.Component {
	...
	constructor(props) {
		super(props);
		this.upwishToggle = this
			.upwishToggle
			.bind(this);
		this.deleteWish = this
			.deleteWish
			.bind(this);
	}
	...
	upwish(unUpwish = false) {
		...
		unUpwish
			? firebaseUtils.unUpwish(wishId, authorUid, uid)
			: firebaseUtils.upwish(wishId, authorUid, uid);
	}
		...
		if (this.props.data.paypalUser) {
			return (
				<a
					...
					target="_blank"
					className={[uikitStyles['uk-button'], uikitStyles['uk-button-default'], styles.donateButton].join(' ')}>
					Donate
        </a>
			)
		}
	...
	render() {
		...
		return <div className={[styles.wishContainer].join(' ')}>
			...
			<div
				className={[uikitStyles['uk-flex'], uikitStyles['uk-flex-between'], uikitStyles['uk-flex-column']].join(' ')}>
				<div className={[uikitStyles['uk-flex']].join(' ')}>
					<div
						className={[uikitStyles['uk-flex'], uikitStyles['uk-margin-small-right'], styles.upwishContainer].join(' ')}
						onClick={this.upwishToggle}>
						<div
							className={[
								styles.upwishButton, userHasWished
									? styles.upwished
									: ''
							].join(' ')}>
							<img
				...
				<div
					...
					<div>
						<TimeAgo>{this.props.data.createdAt}</TimeAgo>
						{' | '}
						<Link to={`/@${this.props.data.user.uid}`}>
							<span style={{ color: colors.primary.textOn }}>{this.props.data.user.displayName}</span>
						</Link>
					</div>
					<div className={uikitStyles['uk-margin-small-left']}>
